    ...
    <div class="card card-accent-secondary">
        ...
        <div class="card-header">
            ...
            <div class="card-actionbar">
                @can('create products')
                    <a href="{{ route('vanilo.paper.create') }}" class="btn btn-sm btn-outline-success float-right">
        ...
        <div class="card-block">
            <table class="table table-striped table-hover">
                ...
                @foreach($products as $product)
                    <tr>
                        <td>
                            @can('view products')<a href="{{ route('vanilo.paper.show', $product) }}">@endcan
                        ...
                        <td>
                            @can('edit products')
                                <a href="{{ route('vanilo.paper.edit', $product) }}"
                                   class="btn btn-xs btn-outline-primary btn-show-on-tr-hover float-right">{{ __('Güncelle') }}</a>
                            @endcan
                            @can('delete products')
                                {!! Form::open(['route' => ['vanilo.paper.destroy', $product],
                                        'method' => 'DELETE',
                                        'data-confirmation-text' => __(':name adlı kayıtı silmek istediğinize emin mnisiniz??', ['name' => $product->name])
                                    ])
                                !!}
                                <button class="btn btn-xs btn-outline-danger btn-show-on-tr-hover float-right">{{ __('Sil') }}</button>
                                {!! Form::close() !!}
                            @endcan
                        </td>
                    </tr>
