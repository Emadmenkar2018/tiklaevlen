@extends('appshell::layouts.default')

@section('title')
    {{ __('Websiteler') }}
@stop

@section('content')

    <div class="card card-accent-secondary">

        <div class="card-header">
            @yield('title')

            <div class="card-actionbar">
                @can('create products')
                    <a href="{{ route('vanilo.website.create') }}" class="btn btn-sm btn-outline-success float-right">
                        <i class="zmdi zmdi-plus"></i>
                        {{ __('Yeni Websitesi') }}
                    </a>
                @endcan
            </div>

        </div>

        <div class="card-block">
            <table class="table table-striped table-hover">
                <thead>
                <tr>
                    <th>&nbsp;</th>
                    <th>{{ __('İsim') }}</th>
                    <th>{{ __('Satışlar') }}</th>
                    <th>{{ __('Kategorilendirme') }}</th>
                    <th>{{ __('Durum') }}</th>
                    <th style="width: 10%">&nbsp;</th>
                </tr>
                </thead>

                <tbody>
                <?php $defaultThumbnail = 'data:image/png;base64,'; ?>
                @foreach($products as $product)
                    <tr>
                        <td>
                            @can('view products')<a href="{{ route('vanilo.website.show', $product) }}">@endcan
                                <img src="{{ $product->getThumbnailUrl() ?: $defaultThumbnail }}" class="mw-100" style="height: 2.5em;" />
                            @can('view products')</a>@endcan
                        </td>
                        <td>
                            <span class="font-lg mb-3 font-weight-bold">
                                @can('view products')<a href="{{ route('vanilo.website.show', $product) }}">@endcan
                                    {{ $product->name }}
                                @can('view products')</a>@endcan
                            </span>
                            <div class="text-muted" title="{{ __('SKU') }}">{{ $product->sku }}</div>
                        </td>
                        <td>
                            <span class="mb-3">
                                {{ $product->units_sold }} {{ __('units sold') }}
                            </span>
                            <div class="text-muted" title="{{ $product->last_sale_at ? $product->last_sale_at->format(__('Y-m-d H:i')) : '' }}">
                                {{ __('Last sale') }}
                                {{ $product->last_sale_at ? $product->last_sale_at->diffForHumans() : __('never') }}
                            </div>
                        </td>
                        <td>
                            <div class="mt-2">
                                @foreach($product->taxons as $taxon)
                                    <span class="badge badge-pill badge-secondary">
                                        {{ $taxon->name }}
                                    </span>
                                @endforeach
                            </div>
                        </td>
                        <td>
                            <div class="mt-2">
                                <span class="badge badge-pill badge-{{$product->is_active ? 'success' : 'secondary'}}">{{ $product->state->label() }}</span>
                            </div>
                        </td>
                        <td>
                            @can('edit products')
                                <a href="{{ route('vanilo.website.edit', $product) }}"
                                   class="btn btn-xs btn-outline-primary btn-show-on-tr-hover float-right">{{ __('Edit') }}</a>
                            @endcan
                            @can('delete products')
                                {!! Form::open(['route' => ['vanilo.website.destroy', $product],
                                        'method' => 'DELETE',
                                        'data-confirmation-text' => __(':name adlı kayıtı silmek istediğinize emin mnisiniz??', ['name' => $product->name])
                                    ])
                                !!}
                                <button class="btn btn-xs btn-outline-danger btn-show-on-tr-hover float-right">{{ __('Delete') }}</button>
                                {!! Form::close() !!}
                            @endcan
                        </td>
                    </tr>
                @endforeach
                </tbody>

            </table>

            @if($products->hasPages())
                <hr>
                <nav>
                    {{ $products->links() }}
                </nav>
            @endif

        </div>
    </div>

@stop
